...
void CX2EffectToolMain::InitDialogEffectSet_List()
{
	...
	pDlg->AddButton( IDC_BUTTON_LOAD_EFFECT_SET_WHOLE, L"이펙트셋 열기", iX, iY, iWidth, iHeight );
	iY += iHeight + 3;
...
void CX2EffectToolMain::OnFrameMove_CameraShake()
{
	...
	CKTDGCamera* pCamera = NULL;
	if( NULL != m_pGame && NULL != m_pGame->GetX2Camera() )
	{
		pCamera = m_pGame->GetX2Camera()->GetCamera();
	}
...
void CX2EffectToolMain::On_IDC_BUTTON_SAVE_EFFECT_SET_WHOLE()
{
	if( NULL == m_pFileManager )
		return;
...
void CX2EffectToolMain::ON_IDC_EDIT_EFFECT_SET_LIMIT_DISTANCE( int iValue_ )
{
	...
	if( NULL != pCameraData )
	{
		pCameraData->m_iLimitDistanceSq = iValue_ ;
	}
}
...
void CX2EffectToolMain::SaveWhole() // 전체 저장
{
	// 이펙트셋 저장
	On_IDC_BUTTON_SAVE_EFFECT_SET_WHOLE();				
