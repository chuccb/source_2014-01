...
//[NOTE] 중국용 라이브러리만 남겼습니다. 디파인은 정확하지만 컴파일상의 오류로 다른 라이브러리가 적용 됬을수도 있으니까
...
//{{김준환 서버시간 받아오기
	...
	{
		 KTDGUIMsgBox( D3DXVECTOR2(250,300), L"서버시간 받기 오류", g_pMain->GetNowState() );
	}
#endif  SERV_SERVER_TIME_GET
...
#	endif REFACTORING_BY_TOOL_TEAM
	...
	{
		switch(g_pMain->GetNowStateID())	//해당 지역에 있기 전에는 안내창 생성을 미룬다.
		{
			//마을 및 거래 광장
		case XS_VILLAGE_MAP:
		case XS_SQUARE_GAME:
...
bool CX2Main::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	switch( uMsg )
	{
		...
		case XGM_QUIT_GAME:
			...
			break;
			//{{ 오현빈 // 2012-03-20 // UI에 사용된 다이얼로그를 UI 닫을 때 해제 하도록 만들기 위해 추가			
...
// 						iMapId = g_pData->GetMyUser()->GetSelectUnit()->GetUnitData()->m_nMapID;
					...
					if(g_pData->GetLocationManager()->GetVillageMapTemplet((SEnum::VILLAGE_MAP_ID)iMapId) == NULL)
					{
						...
						//m_pDLGMsgBox = g_pMain->KTDGUIMsgBox( D3DXVECTOR2(250,300), L"마을로 이동하고 있습니다.", this );	
					}	
					else
					{					
...
#ifdef ELSWORD_NEW_BEGINNING
			if( g_pMain->GetNowStateID() == CX2Main::XS_BEGINNING ) // 서버 선택창으로 가면 모든 메세지 팝업
#else
			if( g_pMain->GetNowStateID() == CX2Main::XS_SERVER_SELECT ) // 서버 선택창으로 가면 모든 메세지 팝업
#endif ELSWORD_NEW_BEGINNING
			{
...
// 			if ( m_vecCheckKom.empty() )
// 				MessageBoxA( g_pKTDXApp->GetHWND(), "체크 완료!", NULL, MB_OK );	
//#endif
		}
	}
