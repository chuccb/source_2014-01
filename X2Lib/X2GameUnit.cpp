...
, m_hSeqEnchant_ArrowOfExplosion( INVALID_PARTICLE_HANDLE )			/// 폭발의 화살 이모티콘 핸들
, m_hStartOfDelayedFiringOverlap( INVALID_PARTICLE_HANDLE )			/// 지연의 신호탄 중첩수 출력 이모티콘
, m_hActiveStartOfDelayedFiringEDT( CX2EffectSet::INVALID_HANDLE )	/// 지연의 신호탄 적용중 확인 이펙트
...
#ifdef SERV_NEW_DEFENCE_DUNGEON // 적용날짜: 2013-04-14 // 해외팀 디파인 추가
...
void CX2GameUnit::CommonFrameMoveStateAbnormalityStartOfDelayedFiring()
{
	...
	for( UINT i = 0; i < GetStartOfDelayedFiringData().size(); ++i )
	{
		...
		if( 0.f < pArrowData[i]->m_fCoolTime )			/// 적용된 화살들의 쿨타임 설정
		{
			pArrowData[i]->m_fCoolTime -= m_fElapsedTime;

			if( 0.f >= pArrowData[i]->m_fCoolTime )		/// 쿨타임 종료시 폭발 동작 실행( 전체 폭발 동작 적용된 화살에 한해서 )
			{
				...
				if( NULL != pAttacker )
				{
					...
					D3DXVECTOR3 vPosOffSet = pArrowData[i]->m_vExplosionOffSet;		/// 폭파 위치 오프셋
					...
					if( CX2DamageEffect::DET_ARROW_OF_EXPLOSION_READY == pArrowData[i]->m_iType )			/// 폭발의 화살
					{
						/// 폭발 이펙트 생성( 폭발의 화살 )
					...
					else if( CX2DamageEffect::DET_START_OF_DELAYED_FIRING_READY == pArrowData[i]->m_iType )	/// 지연의 신호탄
					{
						/// 폭발 이펙트 생성( 지연의 신호탄 )
...
void CX2GameUnit::ProcessTransitDebuff( const CX2DamageManager::DamageData* pDamageData_, CX2GameUnit* pTargetUnit_ )
{
	if ( NULL != pTargetUnit_ && pTargetUnit_->GetNowHp() > 0.0f )
	{
		...
		{
			// 전이 가능한 최대 개수만큼 전이했다면 체크하지 않음.
			if( iTransitDebuffNum >= iMaxAccumulation )
				return false;

			// 해당 EDT의 시간이 0.f 초과했다면 전이
...
{	
	...
	if ( false == pairReturn.second )	// 이미 같은 키를 가진 element가 있다면
		pairReturn.first->second += Damage_;		// 더한다.
}
...
void CX2GameUnit::Weapon::InitUpgradeWeaponParticle()
{
	...
	{
		CX2Unit* pUnit = GetOwnerUnit().GetUnit();		/// 해당 무기를 적용중인 유닛
		CX2Item* pItem = GetOwnerUnit().GetUnit()->GetInventory()->GetEquippingItemByEquipPos( CX2Unit::EP_WEAPON_HAND, false );	/// 해당 무기의 아이템 정보
