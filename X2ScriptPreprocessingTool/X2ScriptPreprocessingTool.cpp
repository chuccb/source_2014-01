...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    {
        if ( wcsicmp( argv[i], L"-exec" ) == 0 && i + 1 < argc )
        {
            vecExec.push_back( argv[i+1] );
            ++i;
        }
        else if ( wcsicmp( argv[i], L"-in" ) == 0 && i + 1 < argc )
        {
            vecIn.push_back( argv[i+1] );
            ++i;
        }
        else if ( wcsicmp( argv[i], L"-out" ) == 0 && i + 1 < argc )
        {
            wstrOut = argv[i+1];
            ++i;
        }
    ...
    for( unsigned u = 0; u != vecExec.size(); ++u )
    {
        ...
        if( g_pKTDXApp->GetLuaBinder()->DoMemory( (const char*)pFile, dwSize ) == E_FAIL )
        {
