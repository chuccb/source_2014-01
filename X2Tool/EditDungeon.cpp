...
void CEditDungeon::ResetDungeon( CX2Dungeon::DUNGEON_ID eDungeonID )
{
	m_iSelectedStage = 0;
	m_iSelectedReadyNpc = -1;
				...
				if( pNpcData->m_vecStartPos[j] == -1 )
 				...
 				if( pDungeon->GetNowStageIndex() == m_iSelectedStage )
 				{
					CKTDGLineMap *pLineMap = g_pX2Game->GetWorld()->GetLineMap();
 					map<int, D3DXVECTOR3>::iterator it = pLineMap->GetStartPosMap().find( pNpcData->m_vecStartPos[j] );
 					if( it == pLineMap->GetStartPosMap().end() || pNpcData->m_vecStartPos[j] == -1 )
 						bInValidStartPos = true;
 				}
			}
...
char* CEditDungeon::LoadLuaFunction(CX2Dungeon::DUNGEON_ID eDungeonID)
{		
	SAFE_DELETE_ARRAY( m_pFunctionData );
