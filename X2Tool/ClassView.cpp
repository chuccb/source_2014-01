...
{	
	...
	int iDungeonCount = g_pData->GetDungeonManager()->GetDungeonCount();

	HTREEITEM hRoot = m_wndClassView.InsertItem(_T("Dungeon Data"), 0, 0);
	m_wndClassView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
	for(int i=0; i<iDungeonCount; ++i)
	{		
		CX2Dungeon::DungeonData* pDungeonData = g_pData->GetDungeonManager()->GetDungeonDataAt(i);
		if( pDungeonData != NULL )
		{
			HTREEITEM hClass = hClass = m_wndClassView.InsertItem( pDungeonData->m_DungeonName.c_str(), 1, 1, hRoot);			
			m_wndClassView.SetItemData(hClass, (DWORD_PTR)pDungeonData);
...
{
	if ( WM_LBUTTONDBLCLK == pMsg->message && pMsg->hwnd == m_wndClassView.m_hWnd )
	{
		...
		{		
			...
			pMain->ResetGame( (CX2Dungeon::DUNGEON_ID)pDungeonData->m_DungeonID );
		}
	}
