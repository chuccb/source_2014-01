...
HRESULT CX2ToolMain::OnFrameMove( double fTime, float fElapsedTime )
{
	...
	if( g_pX2Game != NULL )
	{
		...
		if( pMain != NULL && g_pX2Game->GetX2Camera() != NULL && g_pX2Game->GetX2Camera()->GetCamera() != NULL )
		{
	...
	if( g_pX2Game != NULL && m_bEditLine == true )
	{
		...
		for( UINT i=0; i<pMain->m_wndStartPosition.m_LineStartPositionDlg.vecTeamStartPos.size(); ++i)
		{
			...
			CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData(sp.iLineIndex);
			if( pLineData != NULL )
			{
	...
	{		
		...
		if( m_bEditLine == true )
		{
			...
			if( m_bFillet == true )
			{
				m_iVertPivot = - 1;
				CKTDGLineMap *pLineMap = g_pX2Game->GetWorld()->GetLineMap();
				CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData( m_vFilletLine[0] );
				CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData( m_vFilletLine[1] );
						...
						if( m_bMoveAxis == true && (m_bMoveAxisX == true || m_bMoveAxisY == true || m_bMoveAxisZ == true) )
						{
							...
							for(UINT i=0; i<m_vecLineVertex.size(); ++i)
							{
								if( m_vecLineVertex[i].m_bSelected == true )
								{
									...
									{
										CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData( m_vecLineVertex[i].m_vecLineInfo[j].m_iLineIndex );
										if( pLineData != NULL )
										{
...
void CX2ToolMain::SetCameraView(int iView, D3DXVECTOR3 vFocus)
{
	...
	if( pGameCamera != NULL )
	{
		CKTDGCamera *pCamera = pGameCamera->GetCamera();
		...
		if( pCamera != NULL )
		{
			...
			case 0:	// front
				{
					vEye.z -= 3500.f;
					pCamera->Point( vEye.x, vEye.y, vEye.z, vLookAt.x, vLookAt.y, vLookAt.z, 0.f, 1.f, 0.f );
			...
			case 2: // top
				{
					vEye.y += 3500.f;
					pCamera->Point( vEye.x, vEye.y, vEye.z, vLookAt.x, vLookAt.y, vLookAt.z, 0.f, 0.f, 1.f );
...
/** @function 	: CreateGame
	@brief 		: 게임 생성
*/
void CX2ToolMain::CreateGame( CX2Dungeon::DUNGEON_ID eDungeonId, bool bReset )
{
	...
	pUnit->GetUnitData()->m_UserSkillTree.SetAcquiredSkill( vecSkillAcquired );
	pUnit->GetUnitData()->m_UserSkillTree.SetEquippedSkill( m_tSkillData.m_aEquippedSkill, m_tSkillData.m_aEquippedSkillSlotB );
...
// 	}
	...
	g_pData->GetGameUDP()->AddPeer( pSlotData->m_pUnit->GetUID(), g_pData->GetGameUDP()->GetMyIP(), g_pData->GetGameUDP()->GetMyExtPort() );
...
void CX2ToolMain::ResetGame(CX2Dungeon::DUNGEON_ID eDungeonId)
{
	g_pX2Game->Release();
	SAFE_DELETE(g_pX2Game);
		...
		if( m_vecLineVertex[i].m_bSelected == true )
		{
			...
			for(UINT j=0; j<m_vecLineVertex[i].m_vecLineInfo.size(); ++j)
			{
				...
				if( lineVertInfo.m_bStartPos == false )
					pBeforeLineData = pLineMap->GetLineData( lineVertInfo.m_iLineIndex );
				else
					pNextLineData = pLineMap->GetLineData( lineVertInfo.m_iLineIndex );
			}
			...
			if( m_vecLineVertex[i].m_vecLineInfo.size() == 1 )
			{
				...
				{
					CKTDGLineMap::LineData *pMoreNextLineData = pLineMap->GetLineData(pNextLineData->nextLine);
				...
				{
					CKTDGLineMap::LineData *pMoreBeforeLineData = pLineMap->GetLineData(pBeforeLineData->beforeLine);
...
void CX2ToolMain::DeleteLine()
{
	...
	for(int i=0; i<pLineMap->GetNumLineData(); ++i)
	{
		...
		if( pLineData->m_bSelected == true )
		{
			...
			CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData(iBeforeIndex);
			CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData(iNextIndex);
...
int CX2ToolMain::AddVertex(D3DXVECTOR3 vAddPos, int lineIndex)	// 라인대상
{
	...
	CKTDGLineMap::LineData *pBeforeLineData = pLineMap->GetLineData(lineIndex);
	CKTDGLineMap::LineData *pAfterLineData = pLineMap->GetLineData(pBeforeLineData->nextLine);
...
void CX2ToolMain::SetMakeFirst()
{
	...
	for(UINT i=0; i<m_vecLineVertex.size(); ++i)
	{
		...
		CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData(iStartLine);
		while( pLineData != NULL )
		{
			...
			pLineData = pLineMap->GetLineData(pLineData->nextLine);
		}
	}
...
void CX2ToolMain::SetWeld()
{
	...
	{
		CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData(m_vecLineVertex[m_vecWeldIndex[0]].m_vecLineInfo[i].m_iLineIndex);
	...
	{
		CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData(m_vecLineVertex[m_vecWeldIndex[1]].m_vecLineInfo[i].m_iLineIndex);		
	...
	{
		CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData( vecBeforeLine[0] );
		CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData( vecNextLine[0] );
		if( pBeforeLine != NULL && pNextLine != NULL )
		{
...
{	
	...
	if( pLineMap != NULL )
	{
		CKTDGLineMap::LineData *pOldLine = pLineMap->GetLineData(iOldLine);
		CKTDGLineMap::LineData *pNewLine = pLineMap->GetLineData(iNewLine);
		...
		*pNewLine = tempLine;
		...
		// old before/next
		{
			CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData( pOldLine->beforeLine );
			CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData( pOldLine->nextLine );
		}
		// new before/next
		{
			CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData( pNewLine->beforeLine );
			CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData( pNewLine->nextLine );
		}

		CKTDGLineMap::LineData *pLinkLine = pLineMap->GetLineData(pOldLine->beforeLine);
		if( pLinkLine != NULL )
			pLinkLine->nextLine = iOldLine;
		pLinkLine = pLineMap->GetLineData(pOldLine->nextLine);
		if( pLinkLine != NULL )
			pLinkLine->beforeLine = iOldLine;
		pLinkLine = pLineMap->GetLineData(pNewLine->beforeLine);
		if( pLinkLine != NULL )
			pLinkLine->nextLine = iNewLine;
		pLinkLine = pLineMap->GetLineData(pNewLine->nextLine);
		if( pLinkLine != NULL )
...
void CX2ToolMain::SetVertexPos(D3DXVECTOR3 vPos, D3DXVECTOR3 vApply, bool bMulti)
{
	...
	if( bMulti == false )
	{
		...
		{			
			...
			{
				CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData( m_vecLineVertex[m_iVertPivot].m_vecLineInfo[i].m_iLineIndex );
...
void CX2ToolMain::ReMakeLineData()
{
	...
	for(UINT i=0; i<m_vecLineVertex.size(); ++i)
	{
		...
		CKTDGLineMap::LineData *pBeforeLine = pLineMap->GetLineData(iBefore);		
		if( pBeforeLine != NULL )
		{
			if( pBeforeLine->nextLine != iNext )
				pBeforeLine->nextLine = iNext;
		}
		CKTDGLineMap::LineData *pNextLine = pLineMap->GetLineData(iNext);
		if( pNextLine != NULL )
		{
...
void CX2ToolMain::InitFillet()
{
	...
	for(UINT i=0; i<m_vecLineVertex.size(); ++i)
	{
		if( m_vecLineVertex[i].m_bSelected == true && m_vecLineVertex[i].m_vecLineInfo.size() >= 2 )
		{
			...
			if( m_vecLineVertex[i].m_vecLineInfo[0].m_bStartPos == false )
				pBeforeLine = pLineMap->GetLineData( m_vecLineVertex[i].m_vecLineInfo[0].m_iLineIndex );
			else
				pNextLine = pLineMap->GetLineData( m_vecLineVertex[i].m_vecLineInfo[0].m_iLineIndex );
			if( m_vecLineVertex[i].m_vecLineInfo[1].m_bStartPos == false )
				pBeforeLine = pLineMap->GetLineData( m_vecLineVertex[i].m_vecLineInfo[1].m_iLineIndex );
			else
				pNextLine = pLineMap->GetLineData( m_vecLineVertex[i].m_vecLineInfo[1].m_iLineIndex );
...
void CX2ToolMain::ReArrangeLine()
{
	...
	for(UINT i=0; i<m_vecLineVertex.size(); ++i)
	{
		...
		if( tl.m_vecLineInfo.size() == 1 && tl.m_vecLineInfo[0].m_bStartPos == true )
		{
			...
			while( iNextLine >= 0 )
			{
				CKTDGLineMap::LineData *pLineData = pLineMap->GetLineData( iNextLine );
				if( pLineData != NULL )
				{
