...
/// <summary>
/// Explicit lifecycle state machine for a battle-field room.
/// The transition graph follows the existing KncWX2ServerCS runtime implementation.
/// </summary>
public sealed class RoomStateMachine
{
    ...
    public bool TryTransition(RoomState next)
    {
    ...
    public void Force(RoomState state)
    {
        ...
        if (previous != state)
            Transitioned?.Invoke(previous, state);
    }
