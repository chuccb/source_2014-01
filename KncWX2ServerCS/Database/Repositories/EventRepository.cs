...
public sealed class EventRepository
{
    ...
    public async Task<int> Update30MinuteAsync(long unitUid, short eventType, DateTime endDate, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var rows = exists
                ? await ExecuteAsync(tx, "UPDATE GIs30Min SET RegDate=$regDate WHERE UnitUID=$unitUid AND EventType=$eventType;", cancellationToken, ("$regDate", Format(ToSmallDateTime(endDate))), ("$unitUid", unitUid), ("$eventType", eventType))
                : await ExecuteAsync(tx, "INSERT INTO GIs30Min(UnitUID, EventType, RegDate) VALUES($unitUid,$eventType,$regDate);", cancellationToken, ("$unitUid", unitUid), ("$eventType", eventType), ("$regDate", Format(ToSmallDateTime(endDate))));
    ...
    public async Task<int> InsertTrainingCenterClearAsync(long unitUid, int tcid, DateTime regDate, CancellationToken cancellationToken = default)
    {
        ...
        if (await ExistsAsync(unitUid, tcid, cancellationToken).ConfigureAwait(false)) return -2;
        await using var tx = (SqliteTransaction)await _database.Connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false);
        try
        {
    ...
    private async Task<bool> UnitExistsAsync(long unitUid, CancellationToken ct)
    {
        ...
        c.CommandText = "SELECT EXISTS(SELECT 1 FROM GUnit WHERE UnitUID=$unitUid AND Deleted=0);";
        c.Parameters.AddWithValue("$unitUid", unitUid);
        return Convert.ToInt64(await c.ExecuteScalarAsync(ct).ConfigureAwait(false)) != 0;
    }
    private async Task<bool> ExistsAsync(long unitUid, int tcid, CancellationToken ct)
    {
        await using var c = _database.Connection.CreateCommand();
        c.CommandText = "SELECT EXISTS(SELECT 1 FROM GTrainingCenter WHERE UnitUID=$unitUid AND TCID=$tcid);";
        c.Parameters.AddWithValue("$unitUid", unitUid); c.Parameters.AddWithValue("$tcid", tcid);
        return Convert.ToInt64(await c.ExecuteScalarAsync(ct).ConfigureAwait(false)) != 0;
    }
