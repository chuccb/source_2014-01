...
public static class KUnitInfoSerializer
{
    ...
    public static bool Get(this KSerializer serializer, KUnitInfo value, KUnitInfoWireOptions? options = null)
    {
        ...
        if (!serializer.Get(out int win) || !serializer.Get(out int lose) || !serializer.GetMap(value.DungeonClear, ReadInt, ReadDungeonClear) || !serializer.GetMap(value.TCClear, ReadInt, ReadTCClear)) return false;
        value.Win=win; value.Lose=lose;
        if (options.LimitedDungeonPlayTimes && !serializer.GetMap(value.DungeonPlay, ReadInt, ReadDungeonPlay)) return false;
        if (!serializer.GetMap(value.EquippedItem, ReadInt64, s => ReadInventoryItem(s,options)) || !serializer.Get(value.UnitSkillData, options) || !serializer.Get(out bool isParty) || !serializer.Get(out int spiritMax) || !serializer.Get(out int spirit) || !serializer.Get(out bool isGameBang)) return false;
        value.IsParty=isParty; value.SpiritMax=spiritMax; value.Spirit=spirit; value.IsGameBang=isGameBang;
        if (options.PcBangType && !serializer.Get(out sbyte pcBangType)) return false; if (options.PcBangType) value.PcBangType=pcBangType;
        ...
        if (options.GuildTest && !serializer.Get(value.UserGuildInfo)) return false;
        return GetTrailingFields(serializer,value,options);
    }

    private static bool PutBase(KSerializer serializer, KUnitInfo value) => serializer.Put(value.UnitUid) && serializer.Put(value.UserUid) && serializer.Put(value.CharacterId) && serializer.Put(value.Level) && serializer.Put(value.Exp) && serializer.PutW(value.Name) && serializer.Put(value.Grade) && serializer.Put(value.State) && serializer.Put(value.Gender) && serializer.Put(value.Age) && serializer.Put(value.Job) && serializer.Put(value.JobLevel) && serializer.Put(value.ExpSeason);
    private static bool GetBase(KSerializer serializer, KUnitInfo value)
    {
        if (!serializer.Get(out long unitUid) || !serializer.Get(out long userUid) || !serializer.Get(out int characterId) || !serializer.Get(out int level) || !serializer.Get(out long exp) || !serializer.GetW(out var name) || !serializer.Get(out byte grade) || !serializer.Get(out byte state) || !serializer.Get(out byte gender) || !serializer.Get(out byte age) || !serializer.Get(out sbyte job) || !serializer.Get(out sbyte jobLevel) || !serializer.Get(out long expSeason)) return false;
        value.UnitUid=unitUid; value.UserUid=userUid; value.CharacterId=characterId; value.Level=level; value.Exp=exp; value.Name=name; value.Grade=grade; value.State=state; value.Gender=gender; value.Age=age; value.Job=job; value.JobLevel=jobLevel; value.ExpSeason=expSeason; return true;
    }
    private static bool PutPvp(KSerializer serializer, KUnitInfo value, KUnitInfoWireOptions options)
    {
        if (options.PvpNewSystem)
        {
            ...
            if (options.PvpSeason2) return serializer.Put(value.Rank) && serializer.Put(value.KFactor) && serializer.Put(value.IsRedistributionUser) && serializer.Put(value.PastSeasonWin);
            return true;
        }
    ...
    private static (bool Ok, long Value) ReadInt64(KSerializer serializer) { var ok=serializer.Get(out long value); return (ok,value); }
}
