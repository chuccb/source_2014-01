...
/// <summary>
/// Compatibility serializer for the native KSerializer format.
/// Numeric values use network byte order; UTF-16 wchar_t payloads retain the
/// native Windows 2-byte representation. Tag bytes are emitted only when
/// BeginWriting/BeginReading is called with tagging=true.
/// </summary>
public sealed class KSerializer
{
    ...
    public bool Get(out string value)
    {
        ...
        if (size > ReadLength) return false;
    ...
    public bool GetWide(out string value)
    {
        ...
        if ((size & 1) != 0 || size > ReadLength) return false;
        if (size == 0) return true;
        byte[] bytes = new byte[size];
        if (!ReadBytes(bytes)) return false;
    ...
    {
        if (bytes.IsEmpty) throw new ArgumentException("Native PutRaw requires len > 0.", nameof(bytes));
        return WriteTag(SerializeTag.RawBytes) && WriteBytes(bytes);
    }

    public bool GetRaw(Span<byte> bytes)
    {
        if (bytes.IsEmpty) throw new ArgumentException("Native GetRaw requires len > 0.", nameof(bytes));
        return ReadAndCheckTag(SerializeTag.RawBytes) && ReadBytes(bytes);
    }
    ...
    public bool Get(KSerBuffer value)
    {
        ...
        if (!Get(out bool compressed)) return false;
        if (length > ReadLength) return false;
        byte[] data = new byte[length];
        if (!GetRaw(data)) return false;
        value.Write(data);
        if (compressed && !value.UnCompress()) return false;
        return true;
    }
    ...
    public bool Put(KPerformerInfo value)
    {
        ...
        foreach (long uid in value.UidList)
            if (!Put(uid)) return false;
        return true;
    }
    ...
    private bool ReadNumber<T>(out T value, SerializeTag tag) where T : unmanaged
    {
        ...
        if (BitConverter.IsLittleEndian)
            bytes[..size].Reverse();
        value = System.Runtime.InteropServices.MemoryMarshal.Read<T>(bytes);
        return true;
    }

    private bool WriteTag(SerializeTag tag)
    {
        return !_tagsEnabled || WriteBytes([(byte)tag]);
    }

    private bool ReadAndCheckTag(SerializeTag expected)
    {
        if (!_tagsEnabled) return true;
        Span<byte> raw = stackalloc byte[1];
        return ReadBytes(raw) && raw[0] == (byte)expected;
    }
