...
public sealed class KSerializer
{
    ...
    public bool Put(KSerBuffer v){if(!WriteTag(SerializeTag.Buffer)||!Put((uint)v.Length))return false;if(v.Length==0)return true;return Put(v.IsCompressed)&&PutRaw(v.Data.Span);}
    public bool Get(KSerBuffer v){v.Clear();if(!ReadAndCheckTag(SerializeTag.Buffer)||!Get(out uint n))return false;if(n==0)return true;if(!Get(out bool compressed)||n>ReadLength||n>int.MaxValue)return false;var b=new byte[(int)n];if(!GetRaw(b))return false;v.Write(b);if(compressed)v.MarkCompressed();return true;}

    public bool Put(KPerformerInfo v){if(!WriteTag(SerializeTag.UserClass)||!Put(v.PerformerId)||!WriteTag(SerializeTag.Set)||!Put((uint)v.UidListSize))return false;foreach(var uid in v.UidList)if(!Put(uid))return false;return true;}
    public bool Get(KPerformerInfo v){if(!ReadAndCheckTag(SerializeTag.UserClass)||!Get(out uint id)||!ReadAndCheckTag(SerializeTag.Set)||!Get(out uint n)||n>KPerformerInfo.MaxUidNum)return false;v.ClearUids();for(uint i=0;i<n;i++){if(!Get(out long uid))return false;v.AddUid(uid);}v.PerformerId=id;return true;}
    ...
    public bool PutEvent(KEvent e){if(!WriteTag(SerializeTag.UserClass)||!Put(e.Destination)||!Put(e.FirstTrace)||!Put(e.LastTrace)||!Put(e.EventId)||!Put(e.Buffer))return false;return true;}
    public bool GetEvent(KEvent e){if(!ReadAndCheckTag(SerializeTag.UserClass)||!Get(e.Destination)||!Get(out long first)||!Get(out long last)||!Get(out ushort id)||!Get(e.Buffer))return false;e.SetData(e.Destination.PerformerId,[first,last],id);return true;}
