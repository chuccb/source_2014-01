...
{
    Console.Error.WriteLine("Usage: KncWX2Server.ServerHost --role <login|center|global|channel|game> [--listen <ip:port>] [--config <path>]");
    return 2;
}

await using var runtime = new ServerRuntime(options.Role.Value);
Console.CancelKeyPress += (_, e) => { e.Cancel = true; _ = runtime.StopAsync(); };
AppDomain.CurrentDomain.ProcessExit += (_, _) => _ = runtime.StopAsync();

Console.WriteLine($"KncWX2Server C# 14: role={options.Role}, listen={options.Listen?.ToString() ?? "disabled"}, config={options.ConfigPath ?? "<default>"}");
...
public enum ServerRole { Login, Center, Global, Channel, Game }

public sealed record ServerOptions(ServerRole? Role, IPEndPoint? Listen, string? ConfigPath)
{
    public static ServerOptions Parse(string[] args)
    {
        ServerRole? role = null; IPEndPoint? listen = null; string? config = null;
        for (var i = 0; i < args.Length; i++)
        {
            switch (args[i])
            {
                case "--role" when i + 1 < args.Length: role = ParseRole(args[++i]); break;
                case "--listen" when i + 1 < args.Length: listen = ParseEndpoint(args[++i]); break;
    ...
    {
        "login" => ServerRole.Login, "center" => ServerRole.Center, "global" => ServerRole.Global,
        "channel" => ServerRole.Channel, "game" => ServerRole.Game,
    ...
    private static IPEndPoint ParseEndpoint(string value)
    {
        ...
        if (split <= 0 || split == value.Length - 1 || !int.TryParse(value[(split + 1)..], out var port) || port is < 1 or > 65535)
