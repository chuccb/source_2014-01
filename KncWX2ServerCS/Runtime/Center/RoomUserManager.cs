...
public sealed class RoomUserManager
{
    ...
    public bool IsOneTeamExist(UserListType type=UserListType.Game){lock(_gate){if(Users(type).Count==0)return true;var first=Users(type).Values.FirstOrDefault();if(first is null)return true;return Users(type).Values.Where(u=>u is not null).All(u=>u.Team==first.Team);}}
    ...
    public bool OpenSlotTeam(int slotId,out int pairedSlotId,UserListType type=UserListType.Game){return SetPairedSlot(slotId,true,out pairedSlotId,type);}
    public bool CloseSlotTeam(int slotId,out int pairedSlotId,UserListType type=UserListType.Game){return SetPairedSlot(slotId,false,out pairedSlotId,type);}
    ...
    public bool DecideWinTeam(byte gameType,out List<int> winTeams){lock(_gate){winTeams=new();if(gameType==1){var scores=new Dictionary<int,(int alive,float hp)>();foreach(var u in _gameUsers.Values){var s=scores.GetValueOrDefault(u.Team);scores[u.Team]=(s.alive+(u.IsDie?0:1),s.hp+u.HP);}var bestAlive=scores.Values.Select(x=>x.alive).DefaultIfEmpty(-1).Max();var bestHp=scores.Where(x=>x.Value.alive==bestAlive).Select(x=>x.Value.hp).DefaultIfEmpty(-1).Max();winTeams.AddRange(scores.Where(x=>x.Value.alive==bestAlive&&x.Value.hp==bestHp).Select(x=>x.Key));return true;}if(gameType==2){var scores=new Dictionary<int,int>();foreach(var u in _gameUsers.Values)if(u.IsPlaying)scores[u.Team]=scores.GetValueOrDefault(u.Team)+u.NumKill;var best=scores.Values.DefaultIfEmpty(-1).Max();winTeams.AddRange(scores.Where(x=>x.Value==best).Select(x=>x.Key));return true;}if(gameType==3){var best=_gameUsers.Values.Where(u=>u.StateMachine.State==RoomUserState.Result).Select(u=>u.NumKill).DefaultIfEmpty(-1).Max();winTeams.AddRange(_gameUsers.Values.Where(u=>u.StateMachine.State==RoomUserState.Result&&u.NumKill==best).Select(u=>u.Team).Distinct());return true;}return false;}}
