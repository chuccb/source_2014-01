...
public sealed class RoomUserManager
{
    ...
    public bool IncreaseTeamNumKill(long unitUid){lock(_gate){var u=GetUserUnsafe(unitUid,UserListType.Game);if(u is null)return false;return true;}}
    ...
    public void StartGame(UserListType type=UserListType.Game){lock(_gate)foreach(var u in Users(type).Values)u.StartGame();}
    public void StartPlay(UserListType type=UserListType.Game){lock(_gate)foreach(var u in Users(type).Values)u.StartPlay();}
    public void StartResult(UserListType type=UserListType.Game){lock(_gate)foreach(var u in Users(type).Values)u.EndPlay();}
    public void EndPlay(UserListType type=UserListType.Game){lock(_gate)foreach(var u in Users(type).Values)u.EndPlay();}
    ...
    private bool AppointNewHostUnsafe(UserListType type,long oldHost){var users=Users(type);foreach(var u in users.Values){u.SetHost(u.UnitUid!=oldHost);if(u.UnitUid!=oldHost)return true;}return false;}
}