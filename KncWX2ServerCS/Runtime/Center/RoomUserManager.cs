...
public sealed class RoomUserManager
{
    ...
    public bool DeleteUser(long unitUid,UserListType type=UserListType.Game){lock(_gate){var users=Users(type);if(!users.TryGetValue(unitUid,out var user))return true;if(!Slots(type).FirstOrDefault(x=>ReferenceEquals(x.User,user))?.Leave()==true)return false;return users.Remove(unitUid);}}
    public bool EnterRoom(RoomUser user,bool considerTeam=true,UserListType type=UserListType.Game){ArgumentNullException.ThrowIfNull(user);lock(_gate){var users=Users(type);if(users.ContainsKey(user.UnitUid))return false;var slots=Slots(type);RoomSlot? slot=null;if(considerTeam)slot=FindEmptyTeamSlotInternal(slots,ChooseTeamInternal(users));slot??=slots.FirstOrDefault(x=>x.IsOpened&&!x.IsOccupied);if(slot is null)return false;if(!AddUserUnsafe(user,type))return false;if(!slot.Enter(user)){users.Remove(user.UnitUid);return false;}if(type==UserListType.Game&&users.Count==1)user.SetHost(true);return true;}}
