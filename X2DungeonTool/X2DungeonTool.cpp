...
bool KX2DungeonTool::LoadDungeon( CX2Dungeon::DUNGEON_ID dungeonID )
{
...
void KX2DungeonTool::LoadNPCMeshs()
{
	...
	if( m_pWorld != NULL )
	{
		...
		StringCchPrintf( wszOutput, 512, L"%.2f", m_pWorld->GetLineMap()->GetLandHeight() );
		...
		int iTempInitialDir;
		for( i=0; i<m_pWorld->GetLineMap()->GetRedTeamStartPosNum(); i++ )
		{
			if( m_pWorld->GetLineMap()->GetRedTeamStartRight(i) == true )
				iTempInitialDir = DIR_RIGHT;
			else
				iTempInitialDir = DIR_LEFT;

			int iLineIndex = m_pWorld->GetLineMap()->GetRedTeamStartLineIndex(i);

			vecTeamStartPos.push_back( STeamStartPos( -1, TEAM_RED, m_pWorld->GetLineMap()->GetRedTeamStartPosition(i), iTempInitialDir, iLineIndex) );
		}

		for( i=0; i<m_pWorld->GetLineMap()->GetBlueTeamStartPosNum(); i++ )
		{
			if( m_pWorld->GetLineMap()->GetBlueTeamStartRight(i) == true )
				iTempInitialDir = DIR_RIGHT;
			else
				iTempInitialDir = DIR_LEFT;

			int iLineIndex = m_pWorld->GetLineMap()->GetBlueTeamStartLineIndex(i);

			vecTeamStartPos.push_back( STeamStartPos( -1, TEAM_BLUE, m_pWorld->GetLineMap()->GetBlueTeamStartPosition(i), iTempInitialDir, iLineIndex) );
		}


		map<int, D3DXVECTOR3>& mapStartPos = m_pWorld->GetLineMap()->GetStartPosMap();
		map<int, D3DXVECTOR3>::iterator it;
		for( it=mapStartPos.begin(); it!=mapStartPos.end(); it++ )
		{
			if( true == m_pWorld->GetLineMap()->GetStartRight( (int)it->first ) )
				iTempInitialDir = DIR_RIGHT;
			else
				iTempInitialDir = DIR_LEFT;

			int iLineIndex = m_pWorld->GetLineMap()->GetStartLineIndex( (int)it->first );

			m_abKeyUsed[ (int)it->first ] = true;
			vecTeamStartPos.push_back( STeamStartPos( (int)it->first, TEAM_NONE, m_pWorld->GetLineMap()->GetStartPosition((int)it->first), iTempInitialDir, iLineIndex ) );
