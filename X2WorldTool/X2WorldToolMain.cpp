...
void CX2WorldToolMain::ResetUserUnitStateListBox()
{
	...
	map<UCHAR, CX2GUUser::UserUnitStateData>& mapStateList = g_pX2Game->GetMyUnit()->GetStateList();
	map<UCHAR, CX2GUUser::UserUnitStateData>::iterator it;
	
	for( it=mapStateList.begin(); it!=mapStateList.end(); it++ )
	{
		int* pStateIndex = new int;
		*pStateIndex = (int)it->first;
		CX2GUUser::UserUnitStateData& stateData = (CX2GUUser::UserUnitStateData&)it->second;
